...
export async function GET(request: NextRequest) {
  try {
    ...
    const perspectiveCompetitors =
      PERSPECTIVE_COMPETITORS[
        perspective as keyof typeof PERSPECTIVE_COMPETITORS
      ] || [];
    ...
    if (perspectiveCompetitors.length > 0) {
      ...
      console.log(
        "🎯 Competidores da perspectiva:",
        perspectiveComps?.map((c) => c.name),
      );
    }
...
async function calculateMetricsWithCount(
  supabase: any,
  params: {
    perspective: string;
  ...
  const buildBaseQuery = () => {
    let query = supabase
      .from("ads")
      .select(
        "competitor_id, asset_type, competitors!ads_competitor_id_fkey(name)",
        { count: "exact" },
      )
      .in("asset_type", ["video", "image"]);
  ...
  try {
    ...
    const by_asset_type = Object.entries(assetCounts).map(
...
function extractFeesAndOffers(ads: any[]): {
  ...
  ads.forEach((ad) => {
    ...
    const percentMatches =
      textContent.match(/(\d{1,2}(?:[.,]\d{1,2})?)\s*%/g) || [];
