...
function sendChunk(controller: ReadableStreamDefaultController, chunk: StreamingChunk) {
...
function sendFinal(controller: ReadableStreamDefaultController, sessionId: string) {
...
async function handleAgentStreaming(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Erro no streaming do agente'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionId = chatRequest.sessionId || `session_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`
