...
export function extractRatesFromAd(ad: any): ExtractedRates {
  ...
  const text = `${ad.description || ''} ${ad.extracted_text || ''} ${ad.tags?.join(' ') || ''}`.toLowerCase();
  
  // Padrões de taxa
  const patterns = {
    credit: /(?:crédito|credit).*?(\d+[,.]?\d*%|\d+[,.]?\d*\s*\+\s*\$?\d+[,.]?\d*)/gi,
    debit: /(?:débito|debit).*?(\d+[,.]?\d*%|zero|grátis|gratuito|free)/gi,
    pix: /pix.*?(\d+[,.]?\d*%|zero|grátis|gratuito|free)/gi,
    monthly: /(?:mensalidade|mensal|monthly).*?(r\$\s*\d+[,.]?\d*|\$\d+[,.]?\d*|\d+[,.]?\d*|zero|grátis|gratuito|free)/gi,
    anticipation: /(?:antecipação|antecip|advance).*?(\d+[,.]?\d*%)/gi
  };
  
  // Taxa zero patterns - casos especiais
  if (text.includes('taxa zero') || text.includes('sem taxa') || text.includes('no fees')) {
  ...
  Object.entries(patterns).forEach(([key, pattern]) => {
    ...
    if (matches && matches.length > 0) {
      ...
      const match = matches.find(m => 
        /\d/.test(m) || 
        /zero|grátis|gratuito|free/i.test(m)
      );
      
      if (match) {
        // Extrair apenas a parte da taxa
        const rateMatch = match.match(/(\d+[,.]?\d*%|\d+[,.]?\d*\s*\+\s*\$?\d+[,.]?\d*|zero|grátis|gratuito|free|r\$\s*\d+[,.]?\d*|\$\d+[,.]?\d*)/i);
        if (rateMatch) {
