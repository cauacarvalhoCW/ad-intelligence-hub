...
const CorporateThemeContext = createContext<
  CorporateThemeContextType | undefined
>(undefined);
...
export function useCorporateTheme() {
  ...
  if (!context) {
    throw new Error(
      "useCorporateTheme must be used within a CorporateThemeProvider",
    );
  }
  return context;
}
...
export function useTheme() {
  ...
  const competitorScope =
    themes[corporateTheme]?.metadata?.competitorScope || [];
...
function CorporateThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const [corporateTheme, setCorporateThemeState] =
    useState<ThemeType>("default");
  ...
  return (
    <CorporateThemeContext.Provider
      value={{ corporateTheme, setCorporateTheme, isLoading }}
    >
      {children}
    </CorporateThemeContext.Provider>
  );
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  return <CorporateThemeProvider>{children}</CorporateThemeProvider>;
}
