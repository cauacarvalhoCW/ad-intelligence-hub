...
export function RateExtractor({ text, title }: RateExtractorProps) {
  ...
  useEffect(() => {
    ...
    const percentageRegex =
      /(\d+(?:\.\d+)?%|taxa\s+zero|zero\s+taxa|sem\s+taxa|gratuito|grátis)/gi;
    const percentages = Array.from(
      new Set(fullText.match(percentageRegex) || []),
    );

    // Extract monetary values (e.g., "R$ 10", "$5", "10 reais")
    const priceRegex =
      /(r\$\s*\d+(?:\.\d+)?|\$\s*\d+(?:\.\d+)?|\d+\s*reais?)/gi;
    ...
    const keywords = rateKeywords.filter((keyword) =>
