            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={timelineTrends}>
                ...
                <Tooltip
                  ...
                  formatter={(value, name) => [value, name === "count" ? "Total Anúncios" : "Com Taxas"]}
                />
                <Line type="monotone" dataKey="count" stroke="#8884d8" strokeWidth={2} />
                <Line type="monotone" dataKey="rateAds" stroke="#82ca9d" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
            ...
            {rateTrends.map((rateTrend) => (
              <div key={rateTrend.rate} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    variant={
                      rateTrend.trend === "up" ? "default" : rateTrend.trend === "down" ? "destructive" : "secondary"
                    }
                  >
                    {rateTrend.trend === "up" ? "↗" : rateTrend.trend === "down" ? "↘" : "→"}
                  </Badge>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">{rateTrend.frequency} ocorrências</p>
                  {rateTrend.change > 0 && (
                    <p className="text-xs text-muted-foreground">
                      {rateTrend.trend === "up" ? "Crescimento" : "Queda"} de {rateTrend.change}%
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-primary">
                {Math.round(timelineTrends.reduce((acc, week) => acc + week.count, 0) / timelineTrends.length)}
              </div>
              <p className="text-sm text-muted-foreground">Anúncios/semana</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-secondary">
                {Math.round(timelineTrends.reduce((acc, week) => acc + week.rateAds, 0) / timelineTrends.length)}
              </div>
              <p className="text-sm text-muted-foreground">Com taxas/semana</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-accent">{rateTrends.filter((r) => r.trend === "up").length}</div>
              <p className="text-sm text-muted-foreground">Taxas em alta</p>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-muted-foreground">{platformTrends[0]?.platform || "N/A"}</div>
