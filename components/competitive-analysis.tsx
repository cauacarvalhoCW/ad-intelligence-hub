...
import { TrendingUp, Target, Zap, AlertTriangle, CheckCircle, XCircle } from "lucide-react"
...
export function CompetitiveAnalysis({ ads, competitors, selectedCompetitor }: CompetitiveAnalysisProps) {
  const competitorInsights = useMemo((): CompetitorInsights[] => {
    return competitors.map((competitor) => {
      const competitorAds = ads.filter((ad) => ad.competitor_id === competitor.id)

      // Calculate sentiment score based on keywords
      const positiveKeywords = ["grátis", "gratuito", "sem taxa", "cashback", "desconto", "promoção", "bonus"]
      const aggressiveKeywords = ["melhor", "maior", "único", "exclusivo", "revolucionário"]
      ...
      if (aggressiveScore > competitorAds.length * 0.3) marketPosition = "aggressive"
      ...
      let rateCompetitiveness: CompetitorInsights["rateCompetitiveness"] = "medium"
      if (hasZeroRates || hasCashback) rateCompetitiveness = "high"
      else if (ratesOffered.length < competitorAds.length * 0.3) rateCompetitiveness = "low"

      return {
        id: competitor.id,
        name: competitor.name,
        totalAds: competitorAds.length,
        avgRatesOffered: ratesOffered.length / Math.max(competitorAds.length, 1),
        topStrategies: contentThemes.slice(0, 3).map((t) => t.theme),
        sentimentScore: Math.round((sentimentScore / Math.max(competitorAds.length, 1)) * 100),
        marketPosition,
        rateCompetitiveness,
        contentThemes,
      }
  ...
  const selectedInsights = selectedCompetitor ? competitorInsights.find((c) => c.id === selectedCompetitor) : null
                  ...
                  <div key={position} className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {competitorInsights
                        ...
                        .map((competitor) => (
                          <Badge key={competitor.id} variant="outline" className="w-full justify-center">
                            {competitor.name}
                          </Badge>
                        ))}
                    </div>
            ...
            <Card>
              <CardContent className="text-center py-8">
                <p className="text-muted-foreground">
                  Selecione um concorrente nos filtros para ver análise detalhada de estratégias.
                </p>
              </CardContent>
            </Card>
          )}
...
function CompetitorCard({ insights }: { insights: CompetitorInsights }) {
  ...
  const getCompetitivenessIcon = (level: CompetitorInsights["rateCompetitiveness"]) => {
    switch (level) {
      case "high":
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className={getPositionColor(insights.marketPosition)}>
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-muted-foreground">{insights.sentimentScore}%</span>
          </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-sm font-medium">Competitividade de Taxas</span>
...
function SelectedCompetitorAnalysis({ insights }: { insights: CompetitorInsights }) {
  return (
            ...
            <div className="space-y-2">
              {insights.contentThemes.map(({ theme, frequency }) => (
                <div key={theme} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Progress value={(frequency / insights.totalAds) * 100} className="w-20 h-2" />
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <div className="p-3 bg-muted rounded-lg">
              <h5 className="font-medium text-sm mb-1">Posicionamento de Mercado</h5>
              <p className="text-xs text-muted-foreground">
                {insights.marketPosition === "aggressive"
            ...
            <div className="p-3 bg-muted rounded-lg">
              <h5 className="font-medium text-sm mb-1">Competitividade de Taxas</h5>
              <p className="text-xs text-muted-foreground">
