...
export function CreativeTable({ data, isLoading, product, dimension, filters }: CreativeTableProps) {
  ...
  const aggregatedData = useMemo(() => {
    ...
    console.log(`🚨 [CreativeTable] ANTES DA AGREGAÇÃO:`, {
      total_linhas: data.length,
      produtos_unicos: Array.from(new Set(data.map(d => d.product))),
      dimension: dimension,
      primeiras_5_linhas: data.slice(0, 5).map(d => ({ ad_name: d.ad_name, product: d.product, date: d.date, cost: d.cost }))
    });
    
    const result = aggregateByCreative(data, dimension);
    
    console.log(`🚨 [CreativeTable] DEPOIS DA AGREGAÇÃO:`, {
      linhas_agregadas: result.length,
      primeiras_5: result.slice(0, 5).map(r => ({ ad_name: r.ad_name, days_active: r.days_active, cost: r.cost }))
    });
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>📊 Tabela 1 - Soma por Anúncio (Criativo)</CardTitle>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Não há anúncios para exibir com os filtros atuais.
          </p>
        </CardContent>
      </Card>
    );
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <CardDescription>
              {dimension === "daily" 
                ? "Agregado por Criativo + Dia • Mostrando performance diária de cada criativo"
                : `Agregado por Criativo • ${sortedData.length} criativos únicos • Período completo`
              }
            </CardDescription>
          </div>
