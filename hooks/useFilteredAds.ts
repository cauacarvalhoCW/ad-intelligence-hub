...
export function useThemeStats() {
  ...
  const stats = useMemo(() => {
    ...
    const competitorsCount = new Set(filteredAds.map(ad => ad.competitor?.name)).size
    const themeName = themes[currentTheme]?.name || 'Padrão'
    const themeDescription = themes[currentTheme]?.metadata?.description || 'Todos os competidores'
